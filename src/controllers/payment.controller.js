...
const { findOrederItems } = require("../services/orderItem.service");
...
exports.createCheckoutPayment = async (req, res) => {
  try {
    ...
    const orderItem = await findOrederItems(orderId);
...
exports.webhook = async (req, res) => {
  try {
    ...
    if (event.type == "checkout.session.completed") {
      ...
      await updateById({ orderStatus: STATUS.COMPLETED }, { id: orderId });
      await updatePayment(
    ...
    if (event.type == "checkout.session.async_payment_failed") {
      ...
      const orderId = session.metadata.orderId;
      await updateById({ orderStatus: STATUS.FAILED }, { id: orderId });
      await updatePayment(
